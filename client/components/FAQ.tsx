...
export default function FAQ() {
  ...
  return (
    <section id="faqs" className="py-20 lg:py-28 bg-slate-950 border-t border-white/5">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center mb-12 lg:mb-16">
          <div className="inline-flex items-center justify-center p-2.5 rounded-xl bg-brand-purple/10 text-brand-purple-glow mb-4">
            <HelpCircle size={24} className="stroke-[2.2] text-brand-purple" />
          </div>
          <h2 className="font-sans font-extrabold text-3xl tracking-tight text-white mb-3">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-white/5 overflow-hidden transition-all duration-250 hover:border-white/10"
              >
                <h3>
                  <button
                    id={headingId}
                    ...
                    className="w-full flex items-center justify-between text-left p-6 font-bold text-white hover:text-brand-teal transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal focus-visible:ring-inset"
                  >
                    <span className="pr-4">{faq.question}</span>
                    <ChevronDown
                      size={18}
                      className={`text-slate-400 transition-transform duration-300 ${
                        isOpen ? "transform rotate-185 text-brand-teal" : ""
                      }`}
                    />
                  </button>
                </h3>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-6 pb-6 pt-1 text-slate-350 text-sm leading-relaxed border-t border-white/5">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
